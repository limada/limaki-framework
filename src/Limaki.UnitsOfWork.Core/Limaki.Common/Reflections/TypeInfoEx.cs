...
namespace Limaki.Common.Reflections {
    ...
    public class TypeInfoEx {
        ...
        public virtual TypeInfoEx BaseTypeInfo {
            get {
                if (Type.IsClass) {
                    return new TypeInfoEx { Type = Type.BaseType };
                }
                if (Type.IsInterface) {
                    var interfaze = Type.GetInterfaces ().First ();
                    return new TypeInfoEx { Type = interfaze };
                }
                return new TypeInfoEx { Type = typeof (object) };
            }
        }
        ...
        public IEnumerable<MemberInfoEx> MemberInfos {
            get {
                ...
                if (!Type.IsInterface || !HierarchieMembers) {
                    ...
                    return Type.GetProperties (flags).Select (m => new MemberInfoEx { PropertyInfo = m });
                } else {
                    return (new Type[] { Type })
                           .Concat (Type.GetInterfaces ())
                           .SelectMany (i => i.GetProperties ())
                           .Select (m => new MemberInfoEx { PropertyInfo = m });
                }
            }
        }
    }
