...
namespace Limaki.Common.Phonetics
{
    ...
    /// Diacrits-Removing
    /// </summary>
    public static class Diacrits
    {
        public static string FoldToASCII(string word)
        {
            if (string.IsNullOrEmpty(word))
                return word;
            var outChar = new char[word.Length * 3];
            var len = FoldToASCII(word.ToCharArray(), 0, outChar, 0, word.Length);
            Array.Resize(ref outChar, len);
